...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Сформировать исследовательский отчет с EDA-графиками и сравнением моделей."
    )
    parser.add_argument(
        "--out-dir",
        default=str(ARTIFACTS_DIR / "research_report"),
        help="Папка для сохранения графиков и файлов отчета.",
    )
    ...
    print("[1/4] Загрузка таблицы признаков")
    df = load_feature_table()

    print("[2/4] Сохранение EDA-графиков")
    save_eda_report(df, out_dir)

    print("[3/4] Сохранение графиков сравнения метрик")
    result_files = {
    ...
    print("[4/4] Запись исследовательских заметок")
    write_research_notes(out_dir)
    print(f"Исследовательский отчет сохранен в {out_dir}")
